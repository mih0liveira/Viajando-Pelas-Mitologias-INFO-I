...
if (isset($_POST["cadastrar"])) {
    ...
    // Se não veio nada, assume usuário comum
    $idTipoUsuario = isset($_POST["idTipoUsuario"]) ? intval($_POST["idTipoUsuario"]) : 2;

    // 1. Verifica se o email já está cadastrado
    $verifica = $conn->prepare("SELECT idUsuario FROM Usuario WHERE email = ?");
    $verifica->bind_param("s", $email);
    $verifica->execute();
    $verifica->store_result();

    if ($verifica->num_rows > 0) {
        $mensagem = "<div class='alert alert-danger'>Este email já está cadastrado! Tente outro.</div>";
    } else {
        // 2. Faz o cadastro normal
...
if (isset($_POST["login"])) {
    ...
    if ($resultado->num_rows > 0) {
        ...
        if ($senha === $usuario["senha"] || password_verify($senha, $usuario["senha"])) {
            ...
            $_SESSION["idTipoUsuario"] = $usuario["idTipoUsuario"]; // <-- Aqui
